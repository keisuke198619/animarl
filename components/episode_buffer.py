...
from components.prioritize import Experience
import json
...
class ReplayBuffer(EpisodeBatch):
    def __init__(self, scheme, groups, buffer_size, max_seq_length, burn_in_period, args, from_demo, preprocess=None, device="cpu"):
    ...
    def insert_episode_batch(self, ep_batch, demo=False):
        '''if demo:
            print('demo '+str(self.buffer_index))
        else:
            print('RL '+str(self.buffer_index))'''
        if self.buffer_index + ep_batch.batch_size <= self.buffer_size:
            self.update(ep_batch.data.transition_data,
                        slice(self.buffer_index, self.buffer_index + ep_batch.batch_size),
                        slice(0, ep_batch.max_seq_length), mark_filled=False)
            self.update(ep_batch.data.episode_data, # # len:0
                        slice(self.buffer_index, self.buffer_index + ep_batch.batch_size)) 
            self.buffer_index = (self.buffer_index + ep_batch.batch_size)
            self.episodes_in_buffer = max(self.episodes_in_buffer, self.buffer_index)
            
            self.buffer_index = self.buffer_index % self.buffer_size
            #if self.demo_len > 0 and not demo and self.buffer_index == 0:
            #    self.buffer_index = self.demo_len
            #if self.buffer_index == 17:
            #    import pdb; pdb.set_trace()
    ...
    def sample(self, batch_size):
        ...
        if self.episodes_in_buffer == batch_size:
            ...
            assert self.episodes_in_buffer%2 == 0
            # Uniform sampling only atm
            ep_ids = np.random.choice(int(self.episodes_in_buffer//2), int(batch_size//2), replace=False)
            ep_ids = np.repeat(ep_ids[:,None],2,1)*2
            ep_ids[:,1] += 1
            return self[ep_ids.reshape((-1,))]
...
class ReplayBuffer_Prior(EpisodeBatch):
    def __init__(self, scheme, groups, buffer_size, max_seq_length, burn_in_period, args, from_demo, preprocess=None, device="cpu", alpha=0.1):
        ...
        self.burn_in_period = burn_in_period
        self.from_demo = from_demo
        self.proportional = Experience(buffer_size, from_demo, alpha=alpha)
        self.args = args
    ...
    def sample(self, batch_size):
        assert self.on_policy_can_sample(batch_size)
        if self.from_demo:
            idx_ = self.proportional.select(int(batch_size//2))
            idx = np.repeat(np.array(idx_)[:,None],2,1)
            idx[(np.array(idx_)%2==1),0] -= 1
            idx[(np.array(idx_)%2==0),1] += 1
            idx = idx.reshape((-1,))
        else:
    ...
    def update_priority(self, indices, priorities):
        try: self.proportional.priority_update(indices, priorities) # list[8][17]
        except: import pdb; pdb.set_trace()

...
class Animarl_Agent_2vs1(MultiAgentEnv):
    def __init__(
        self,
        n_agents=2,
        time_limit=300,
        time_step=0,
        obs_dim=18, # 12
        env_name='animarl_agent_2vs1',
        stacked=False,
        representation="simple",
        rewards='touch',
        logdir='animarl_agent_dumps',
        args=None,
    ...
    def step(self, actions):
        ...
        self.state = np.concatenate([state_p1.reshape((-1,)),self.cond],0)
        ...
        reward_p1 = self.get_reward_pursuer(next_pos_p1, next_pos_p2, next_pos_e, reward_share)
        reward_p2 = self.get_reward_pursuer(next_pos_p2, next_pos_p1, next_pos_e, reward_share)
        ...
        try: 
            ...
            self.obs = np.concatenate([self.obs,np.repeat(self.cond[:,None], 3, axis=0)],1)
    ...
    def reset(self,initial=None):
        ...
        self.state = np.concatenate([pos_p1,vel_p1,pos_p2,vel_p2,pos_e,vel_e,cond],0)  
        self.obs = np.concatenate([obs,np.repeat(cond[:,None], 3, axis=0)],1)
        self.cond = cond
    ...
    def get_reward_evader(self, abs_pos_own, abs_pos_adv1, abs_pos_adv2):
        ...
        #elif dist1 < 0.1 or dist2 < 0.1:
        #    reward = -1
        if abs_pos_own[0] < -1 or abs_pos_own[1] < -1 or abs_pos_own[0] > 1 or abs_pos_own[1] > 1:
            reward = -10

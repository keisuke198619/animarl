...
class Animarl_Newt_2vs1(MultiAgentEnv):
    ...
    def step(self, actions):
        ...
        abs_u_p1 = self.get_abs_u(action_p1, pos_p1, pos_e, self.speed_p1, sign=1)
        next_pos_p1, next_vel_p1 = self.get_next_own_state(pos_p1, vel_p1, abs_u_p1, \
                                                    self.mass_p1, action_p1[2], self.damping, self.dt) 

        abs_u_p2 = self.get_abs_u(action_p2, pos_p2, pos_e, self.speed_p2, sign=1)
        next_pos_p2, next_vel_p2 = self.get_next_own_state(pos_p2, vel_p2, abs_u_p2, \
                                                    self.mass_p2, action_p2[2], self.damping, self.dt) 
        
        state_adv = [pos_e, pos_p1, vel_p1, pos_p2, vel_p2]
        pos_adv1, _, _, _ = get_order_adv(state_adv,self.n_mate,self.n_adv)      
        abs_u_e = self.get_abs_u(action_e, pos_e, pos_adv1, self.speed_e, sign=-1)
        next_pos_e, next_vel_e = self.get_next_own_state(pos_e, vel_e, abs_u_e, \
                                                    self.mass_e, action_e[2], self.damping, self.dt)

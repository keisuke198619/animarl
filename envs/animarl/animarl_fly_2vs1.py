...
class Animarl_Fly_2vs1(MultiAgentEnv):
    def __init__(
        self,
        n_agents=2,
        time_limit=300,
    ...
    def step(self, actions):
        ...
        state_p1, state_p2, state_e, next_pos_p1, next_pos_p2, next_pos_e = transition_agent(self.state, action_p1, action_p2, action_e, params, boundary='circle')

        self.state = state_p1.reshape((-1,))

        dist_org_p1 = np.array([np.sqrt(np.sum(state_p1[:2]**2))])[None,:]
        dist_org_p2 = np.array([np.sqrt(np.sum(state_p2[:2]**2))])[None,:]
        dist_org_e = np.array([np.sqrt(np.sum(state_e[:2]**2))])[None,:]

        next_obs_p1 = np.concatenate([get_obs_p(state_p1,self.n_mate,self.n_adv),dist_org_p1],1)
        next_obs_p2 = np.concatenate([get_obs_p(state_p2,self.n_mate,self.n_adv),dist_org_p2],1)
        next_obs_e = np.concatenate([get_obs_e(state_e,self.n_mate,self.n_adv),dist_org_e],1)
    ...
    def reset(self,initial=None):
        ...
        dist_org_p1 = np.array([np.sqrt(np.sum(pos_p1**2))])[None,:]
        dist_org_p2 = np.array([np.sqrt(np.sum(pos_p2**2))])[None,:]
        dist_org_e = np.array([np.sqrt(np.sum(pos_e**2))])[None,:]

        obs = get_obs_p(state_p1,self.n_mate,self.n_adv) # obs_p1
        obs = np.concatenate([obs,dist_org_p1],1)
        obs_ = np.concatenate([get_obs_p(state_p2,self.n_mate,self.n_adv),dist_org_p2],1)
        obs = np.concatenate([obs,obs_],0) # obs_p2
        obs_ = np.concatenate([get_obs_e(state_e,self.n_mate,self.n_adv),dist_org_e],1)
    ...
    def get_reward_pursuer(self, abs_pos_own, abs_pos_mate, abs_pos_adv, reward_share):
        ...
        if reward_share == True:
            if dist1 < self.thresh or dist2 < self.thresh:
                reward = 1
            elif np.sqrt(np.square(abs_pos_own).sum()) > self.bound:
    ...
    def get_reward_evader(self, abs_pos_own, abs_pos_adv1, abs_pos_adv2):
        ...
        #    reward = -1
        #el
        if np.sqrt(np.square(abs_pos_own).sum()) > self.bound:
            reward = self.reward_out
    ...
    def get_done(self, abs_pos_own, abs_pos_adv1, abs_pos_adv2, num_step, max_step):
        ...
        if dist1 < self.thresh or dist2 < self.thresh or num_step > max_step or  \
            np.sqrt(np.square(abs_pos_own).sum()) > self.bound or \
            np.sqrt(np.square(abs_pos_adv1).sum()) > self.bound or \
            np.sqrt(np.square(abs_pos_adv2).sum()) > self.bound:
            done = True
        else:
            done = False
